...
export const DEFAULT_CHAIN_ID = KAVA;
...
const KAVA_RPC_PROVIDERS = process.env.REACT_APP_KAVA_RPC_URLS.split(" ");
...
export const KLP_COOLDOWN_DURATION = 900;
...
export const ICONLINKS = {
  2222: {
    KLP: {
      coingecko: "https://www.coingecko.com/en/coins/klp",
      kava: "https://explorer.kava.io/address/0x5d370C8Fb021cfaa663D35a7c26fb59699ff42DA",
    },
    KAVA: {
      coingecko: "https://www.coingecko.com/en/coins/kava",
      kava: "https://explorer.kava.io/address/0xc86c7C0eFbd6A49B35E8714C5f59D99De09A225b",
    },
    ETH: {
...
export const platformTokens = {
  2222: {
    KLP: {
      name: "Kinetix Perps LP",
      symbol: "KLP",
      decimals: 18,
      address: getContract(KAVA, "FeeKlpTracker"),
      imageUrl: "https://res.cloudinary.com/kinetix/image/upload/v1662984581/website-assets/klp-token.png",
    },
  },
};

const supportedChainIds = [KAVA];
...
export function getServerBaseUrl(chainId) {
  let serverBaseUrl = process.env.REACT_APP_API_KAVA_URL;
...
export function getOrderApiBaseUrl(chainId) {
  let serverBaseUrl = process.env.REACT_APP_ORDER_API_KAVA_URL || "";
  if(serverBaseUrl.at(-1) === '/'){
    serverBaseUrl = serverBaseUrl.substring(0,serverBaseUrl.length-1)
  }
  if (!chainId) {
...
export function isTriggerRatioInverted(fromTokenInfo, toTokenInfo) {
  ...
  if (toTokenInfo.isStable || toTokenInfo.isUsdk) return true;
...
export function shouldInvertTriggerRatio(tokenA, tokenB) {
  if (tokenB.isStable || tokenB.isUsdk) return true;
...
export function getExchangeRateDisplay(rate, tokenA, tokenB, opts = {}) {
  ...
  const rateValue = formatAmount(
    rate,
    USD_DECIMALS,
    tokenA.isStable || tokenA.isUsdk ? tokenA.displayDecimals : 4,
    true
  );
  if (opts.omitSymbols) {
    return rateValue;
...
export function getTargetUsdkAmount(token, usdkSupply, totalTokenWeights) {
  if (!token || !token.weight || !usdkSupply) {
    return;
  }
  ...
  return token.weight.mul(usdkSupply).div(totalTokenWeights);
}
...
export function getFeeBasisPoints(
  ...
  if (!token || !token.usdkAmount || !usdkSupply || !totalTokenWeights) {
    return 0;
  }
  ...
  const initialAmount = token.usdkAmount;
  let nextAmount = initialAmount.add(usdkDelta);
  if (!increment) {
    nextAmount = usdkDelta.gt(initialAmount) ? bigNumberify(0) : initialAmount.sub(usdkDelta);
  }

  const targetAmount = getTargetUsdkAmount(token, usdkSupply, totalTokenWeights);
...
export function getBuyKlpToAmount(fromAmount, swapTokenAddress, infoTokens, klpPrice, usdkSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0), feeBasisPoints: 0 };
  if (!fromAmount || !swapTokenAddress || !infoTokens || !klpPrice || !usdkSupply || !totalTokenWeights) {
    return defaultValue;
  }
  ...
  let klpAmount = fromAmount.mul(swapToken.minPrice).div(klpPrice);
  klpAmount = adjustForDecimals(klpAmount, swapToken.decimals, USDK_DECIMALS);

  let usdkAmount = fromAmount.mul(swapToken.minPrice).div(PRECISION);
  usdkAmount = adjustForDecimals(usdkAmount, swapToken.decimals, USDK_DECIMALS);
  ...
  klpAmount = klpAmount.mul(BASIS_POINTS_DIVISOR - feeBasisPoints).div(BASIS_POINTS_DIVISOR);

  return { amount: klpAmount, feeBasisPoints };
}

export function getSellKlpFromAmount(toAmount, swapTokenAddress, infoTokens, klpPrice, usdkSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0), feeBasisPoints: 0 };
  if (!toAmount || !swapTokenAddress || !infoTokens || !klpPrice || !usdkSupply || !totalTokenWeights) {
    return defaultValue;
  }
  ...
  let klpAmount = toAmount.mul(swapToken.maxPrice).div(klpPrice);
  klpAmount = adjustForDecimals(klpAmount, swapToken.decimals, USDK_DECIMALS);

  let usdkAmount = toAmount.mul(swapToken.maxPrice).div(PRECISION);
  usdkAmount = adjustForDecimals(usdkAmount, swapToken.decimals, USDK_DECIMALS);
  const feeBasisPoints = getFeeBasisPoints(
    swapToken,
    usdkAmount,
  ...
  klpAmount = klpAmount.mul(BASIS_POINTS_DIVISOR).div(BASIS_POINTS_DIVISOR - feeBasisPoints);

  return { amount: klpAmount, feeBasisPoints };
}

export function getBuyKlpFromAmount(toAmount, fromTokenAddress, infoTokens, klpPrice, usdkSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0) };
  if (!toAmount || !fromTokenAddress || !infoTokens || !klpPrice || !usdkSupply || !totalTokenWeights) {
    return defaultValue;
  }
  ...
  let fromAmount = toAmount.mul(klpPrice).div(fromToken.minPrice);
  fromAmount = adjustForDecimals(fromAmount, KLP_DECIMALS, fromToken.decimals);

  const usdkAmount = toAmount.mul(klpPrice).div(PRECISION);
  const feeBasisPoints = getFeeBasisPoints(
    fromToken,
    usdkAmount,
    MINT_BURN_FEE_BASIS_POINTS,
...
export function getSellKlpToAmount(toAmount, fromTokenAddress, infoTokens, klpPrice, usdkSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0) };
  if (!toAmount || !fromTokenAddress || !infoTokens || !klpPrice || !usdkSupply || !totalTokenWeights) {
    return defaultValue;
  }
  ...
  let fromAmount = toAmount.mul(klpPrice).div(fromToken.maxPrice);
  fromAmount = adjustForDecimals(fromAmount, KLP_DECIMALS, fromToken.decimals);
...
export function getNextFromAmount(
  ...
  let usdkAmount = fromAmount.mul(fromTokenMinPrice).div(PRECISION);
  usdkAmount = adjustForDecimals(usdkAmount, toToken.decimals, USDK_DECIMALS);
...
export function getNextToAmount(
  ...
  if (toTokenAddress === USDK_ADDRESS) {
  ...
  if (fromTokenAddress === USDK_ADDRESS) {
    ...
    if (ratio && !ratio.isZero()) {
      ...
      return {
        ...
        path: [USDK_ADDRESS, stableToken.address, toToken.address],
        feeBasisPoints: feeBasisPoints0 + feeBasisPoints1,
      };
    }

    // get toAmount for USDK => stableToken
    let toAmount = fromAmount.mul(PRECISION).div(stableToken.maxPrice);
    // apply USDK => stableToken fees
  ...
  let usdkAmount = fromAmount.mul(fromTokenMinPrice).div(PRECISION);
  usdkAmount = adjustForDecimals(usdkAmount, fromToken.decimals, USDK_DECIMALS);
  const swapFeeBasisPoints =
    fromToken.isStable && toToken.isStable ? STABLE_SWAP_FEE_BASIS_POINTS : SWAP_FEE_BASIS_POINTS;
  const taxBasisPoints = fromToken.isStable && toToken.isStable ? STABLE_TAX_BASIS_POINTS : TAX_BASIS_POINTS;
  const feeBasisPoints0 = getFeeBasisPoints(
...
const RPC_PROVIDERS = {
  [KAVA]: KAVA_RPC_PROVIDERS,
};

const FALLBACK_PROVIDERS = {
  [KAVA]: process.env.REACT_APP_KAVA_FALLBACK_PROVIDERS.split(" "),
};
...
export function getExplorerUrl(chainId) {
  if (chainId === KAVA) {
    return process.env.REACT_APP_EXPLORER_KAVA_URL;
  }
  return process.env.REACT_APP_EXPLORER_KAVA_URL;
}
  ...
  contract
    ...
    .catch((e) => {
      ...
      if (
        ...
        failMsg = (
          <div>
            There is not enough KAVA in your account on KAVA to send this transaction.
            <br />
          </div>
        );
...
export function getInfoTokens(
  ...
  for (let i = 0; i < whitelistedTokens.length; i++) {
    ...
    if (vaultTokenInfo) {
      ...
      token.usdkAmount = vaultTokenInfo[i * vaultPropsLength + 2];
      token.redemptionAmount = vaultTokenInfo[i * vaultPropsLength + 3];
      token.weight = vaultTokenInfo[i * vaultPropsLength + 4];
      token.bufferAmount = vaultTokenInfo[i * vaultPropsLength + 5];
      token.maxUsdkAmount = vaultTokenInfo[i * vaultPropsLength + 6];
      ...
      if (token.maxUsdkAmount.eq(0)) {
        token.maxUsdkAmount = DEFAULT_MAX_USDK_AMOUNT;
      }
...
export function getProcessedData(balanceData, supplyData, depositBalanceData, stakingData, aum, nativeTokenPrice) {
  ...
  data.klpSupply = supplyData.klp;
  data.klpPrice =
    data.klpSupply && data.klpSupply.gt(0)
      ? aum.mul(expandDecimals(1, KLP_DECIMALS)).div(data.klpSupply)
      : bigNumberify(0);

  data.klpSupplyUsd = supplyData.klp.mul(data.klpPrice).div(expandDecimals(1, 18));

  data.klpBalance = depositBalanceData.klpInStakedKlp;
  data.klpBalanceUsd = depositBalanceData.klpInStakedKlp.mul(data.klpPrice).div(expandDecimals(1, KLP_DECIMALS));

  data.feeKlpTrackerRewards = stakingData.feeKlpTracker.claimable;
  data.feeKlpTrackerRewardsUsd = stakingData.feeKlpTracker.claimable.mul(nativeTokenPrice).div(expandDecimals(1, 18));

  data.feeKlpTrackerAnnualRewardsUsd = stakingData.feeKlpTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(nativeTokenPrice)
    .div(expandDecimals(1, 18));

  data.klpAprForNativeToken =
    data.klpSupplyUsd && data.klpSupplyUsd.gt(0)
      ? data.feeKlpTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.klpSupplyUsd)
      : bigNumberify(0);
  data.klpAprTotal = data.klpAprForNativeToken;

  data.totalKlpRewardsUsd = data.feeKlpTrackerRewardsUsd;
...
export function isDevelopment() {
  return (
    !window.location.host?.includes("perps.kinetix.finance") &&
    !window.location.host?.includes("perps.kinetix.finance")
  );
}
...
export function getHomeUrl() {
  return "https://perps.kinetix.finance";
}
