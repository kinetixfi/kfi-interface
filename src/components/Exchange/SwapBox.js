...
export default function SwapBox(props) {
  ...
  const maxFromTokenInUSD = useMemo(() => {
    const value = fromTokenInfo.maxUsdkAmount
      ?.sub(fromTokenInfo.usdkAmount)
      .mul(expandDecimals(1, USD_DECIMALS))
      .div(expandDecimals(1, USDK_DECIMALS));
  ...
  const getSwapError = () => {
    ...
    if (!isMarketOrder) {
      if ((toToken.isStable || toToken.isUsdk) && (fromToken.isStable || fromToken.isUsdk)) {
        return ["Select different tokens"];
      }
    ...
    if (
      fromUsdMin &&
      fromTokenInfo.maxUsdkAmount &&
      fromTokenInfo.maxUsdkAmount.gt(0) &&
      fromTokenInfo.usdkAmount &&
      fromTokenInfo.maxPrice
    ) {
      const usdkFromAmount = adjustForDecimals(fromUsdMin, USD_DECIMALS, USDK_DECIMALS);
      const nextUsdkAmount = fromTokenInfo.usdkAmount.add(usdkFromAmount);

      if (nextUsdkAmount.gt(fromTokenInfo.maxUsdkAmount)) {
  ...
  const getLeverageError = () => {
    ...
    if (isLong) {
      ...
      if (fromTokenAddress !== toTokenAddress) {
        ...
        if (toToken && toTokenAddress !== USDK_ADDRESS) {
        ...
        if (
          ...
          const nextUsdkAmount = fromTokenInfo.usdkAmount.add(usdkFromAmount);
          if (nextUsdkAmount.gt(fromTokenInfo.maxUsdkAmount)) {
            return [`${fromTokenInfo.symbol} pool exceeded, try different token`, true, "MAX_USDK"];
          }
        }
      }
  ...
  const renderErrorModal = () => {
    ...
    const inputTokenSymbol = isLong ? fromToken.symbol : shortCollateralToken.symbol;
      ...
      let swapUrl = `https://kinetix.finance/#/swap`;
    let equilibreSwapUrl = `https://equilibrefinance.com/swap?token0=${inputCurrency}&token1=${outputCurrency}`;
    const label =
      ...
      <Modal isVisible={!!modalError} setIsVisible={setModalError} label={label} className="Error-modal font-base">
        <div style={{ padding: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", flexDirection: "column", gap: "10px" }}>
                    <a style={{ textDecoration: "none", color: "#FF7847" }} href={swapUrl} target="_blank" rel="noreferrer">
                        Buy {swapTokenSymbol} on Kinetix
            </a>
            {/* <a
              style={{ textDecoration: "none", color: "#FF7847" }}
              href={equilibreSwapUrl}
              target="_blank"
              rel="noreferrer"
            >
              Buy {swapTokenSymbol} on Equilibre
            </a> */}
          </div>
        </div>
      </Modal>
  ...
  const getPrimaryText = () => {
    ...
    if (isLong) {
      ...
      if (indexTokenInfo && indexTokenInfo.minPrice) {
        ...
        if (fromTokenAddress === USDK_ADDRESS && nextToAmountUsd.lt(fromUsdMin.mul(98).div(100))) {
          return "High USDK Slippage, Long Anyway";
        }
      }
  ...
  const increasePosition = async () => {
    ...
    if (fromAmount && fromAmount.gt(0) && fromTokenAddress === USDK_ADDRESS && isLong) {
  ...
  if (isSwap) {
    if (fromAmount) {
      ...
      if (feeBasisPoints !== undefined) {
        ...
        const feeTokenPrice =
          fromTokenInfo.address === USDK_ADDRESS ? expandDecimals(1, USD_DECIMALS) : fromTokenInfo.maxPrice;
    ...
    <div className="Exchange-swap-box">
      ...
      <div className="Exchange-swap-market-box App-box App-box-border">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }} className="Exchange-info-row">
          ...
          {/* <div className="Exchange-info-label-button">
            <a href="https://perps.kinetix.finance/#/orders_overview" target="_blank" rel="noopener noreferrer">
              Orders Overview ↗
            </a>
          </div> */}
        </div>
